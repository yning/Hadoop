...
public class recommenderGD
{
    ...
    public static class GradientDescentMap extends Mapper<LongWritable,Text,Text,Text>
    {
    ...
    public static class GradientDescentCombiner extends Reducer<Text,Text,Text,Text>
    {
        @Override
    ...
    public static class GradientDescentReducer extends Reducer<Text,Text,Text,Text>
    {
        private long users;
        private long items;
        private int count;
    ...
    public static void main(String[] args) throws Exception 
    {
        ...
        Path outputPath = new Path(args[8]);//output
        Path outputRMSE = new Path(args[9]);//rmse
        Path pqmatrix = new Path(args[10]);//matrix
        Path outputTest = new Path(args[11]);//test
        Path bgdout = null;
        Path rmseout = null;
        //Path prior = null;
        ...
        try {
            ...
            // jobconf is for Batch Gradient Descent
            while(iteration<numIteration) {
                Configuration jobconf = new Configuration();
                if (iteration == 0) DistributedCache.addCacheFile(new Path("matrix/pqmatrix").toUri(), jobconf);
                else {
                ...
                job.setJobName("recommenderGD"+iteration);
                job.setJarByClass(recommenderGD.class);

                job.setMapperClass(GradientDescentMap.class);
                job.setCombinerClass(GradientDescentCombiner.class);
                job.setReducerClass(GradientDescentReduce.class);
                ...
                //File src = new File("output/round"+iteration+"/part-r-00000");
                //File dest = new File("matrix/pqmatrix");
                FileSystem hdfs = FileSystem.get(jobconf);
                FileUtil.copy(hdfs, bgdout, hdfs, pqmatrix, false, true, jobconf);
                //FileUtils.copy(src, dest);
                //This is for computing rmse
                if(iteration>=0) {
                    ...
                    job2.setJarByClass(recommenderGD.class);
                    ...
                    if(exitCode2 == 0) {  
	                    if (iteration >= 1) {
		                    ...
		                    br_current.close();
		                    br_before.close();
		                    if ((diff_error < 0.2 && diff_error > 0.0) | iteration > 10 | diff_error < 0.0) {
                                if (diff_error < 0.0) iteration--;
                                break;
                            }
	                	}            
                    } 
